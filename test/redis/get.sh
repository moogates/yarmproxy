...
query="get key2 key1\r\n"
echo "1.--------- $query -----------"
#printf "$query" | ../yarmnc 127.0.0.1 $YARMPROXY_PORT | grep "VALUE\|END"
echo
...
echo "--------- $query -----------"
printf "$query" | ../yarmnc 127.0.0.1 $YARMPROXY_PORT > tmp
...
echo "2.--------- $query -----------"
printf "$query" | ../yarmnc 127.0.0.1 $YARMPROXY_PORT -w 1 > tmp
cat tmp | grep "VALUE\|END"
echo
