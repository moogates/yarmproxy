    ...
    , keys_count_(query_bulks.total_bulks() - 1)
{
  ...
  LOG_DEBUG << "RedisMgetCommand ctor, query_set_.size=" << query_set_.size()
...
void RedisMgetCommand::OnBackendConnectError(std::shared_ptr<BackendConn> backend) {
  ...
  if (backend == last_backend_) {
    ...
    LOG_WARN << "RedisMgetCommand::OnBackendConnectError last, endpoint="
            << backend->remote_endpoint() << " backend=" << backend;
  } else {
...
void RedisMgetCommand::RotateReplyingBackend(bool success) {
  if (success) {
    ++completed_backends_;
  }
