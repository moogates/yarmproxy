...
{
  LOG_DEBUG << "ParallelGetCommand ctor " << ++single_get_cmd_count;
}

ParallelGetCommand::~ParallelGetCommand() {
  LOG_DEBUG << "ParallelGetCommand dtor " << --single_get_cmd_count;
}

bool ParallelGetCommand::ParseUpstreamResponse() {
  bool valid = true;
...
void ParallelGetCommand::OnForwardResponseReady() {
  ...
  if (!is_forwarding_response_ && backend_conn_->read_buffer_.unprocessed_bytes() > 0) {
    ...
    LOG_DEBUG << "ParallelGetCommand OnForwardResponseReady, backend no data ready to_transfer, waiting to read more data then write down";
  }
}
...
void ParallelGetCommand::DoForwardRequest(const char *, size_t) {
  ...
  for(auto it : backend_map_) {
    BackendTask& bt = it->second;
    it->second->ForwardRequest(bt.sub_cmd_line_.data(), bt.sub_cmd_line_.size(), false);
  }
}
