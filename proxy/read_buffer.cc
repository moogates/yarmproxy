...
void ReadBuffer::inc_recycle_lock() {
  ++recycle_lock_count_;
  LOG_INFO << "ReadBuffer::inc_recycle_lock, this=" << this << " recycle_lock_count_=" << recycle_lock_count_;
}
void ReadBuffer::dec_recycle_lock() {
  if (recycle_lock_count_ > 0) {
    --recycle_lock_count_;
    LOG_INFO << "ReadBuffer::dec_recycle_lock, this=" << this << " recycle_lock_count_=" << recycle_lock_count_;
  } else {
    LOG_WARN << "ReadBuffer::dec_recycle_lock, this=" << this << " bad unlock";
  }
...
void ReadBuffer::try_recycle_buffer() {
  if (recycle_lock_count_ == 0) {
    LOG_INFO << "ReadBuffer try_recycle_buffer(), this=" << this << " memmove is unlocked, try moving offset, PRE: begin=" << processed_offset_
