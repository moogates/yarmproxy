...
RedisGetsCommand::BackendQuery::~BackendQuery() {
}

RedisGetsCommand::RedisGetsCommand(std::shared_ptr<ClientConnection> client,
    ...
    , keys_count_(query_bulks.total_bulks() - 1)
{
  for(auto& it : endpoint_query_map) {
    LOG_DEBUG << "RedisGetsCommand ctor, create query ep=" << it.first << " query=" << it.second;
    query_set_.emplace_back(new BackendQuery(it.first, std::move(it.second)));
  }
  LOG_DEBUG << "RedisGetsCommand ctor, query_set_.size=" << query_set_.size()
            << " count=" << ++redis_mget_cmd_count;
}

RedisGetsCommand::~RedisGetsCommand() {
  for(auto& query : query_set_) {
    if (!query->backend_conn_) {
      LOG_DEBUG << "RedisGetsCommand dtor Release null backend";
    } else {
      LOG_DEBUG << "RedisGetsCommand dtor Release backend";
      backend_pool()->Release(query->backend_conn_);
    }
  }
  LOG_DEBUG << "RedisGetsCommand dtor, cmd=" << this << " count=" << --redis_mget_cmd_count;
}

void RedisGetsCommand::WriteQuery() {
  for(auto& query : query_set_) {
    if (!query->backend_conn_) {
      query->backend_conn_ = AllocateBackend(query->backend_endpoint_);
    }
    LOG_DEBUG << "RedisGetsCommand WriteQuery cmd=" << this
...
void RedisGetsCommand::TryMarkLastBackend(std::shared_ptr<BackendConn> backend) {
...
bool RedisGetsCommand::TryActivateReplyingBackend(std::shared_ptr<BackendConn> backend) {
...
void RedisGetsCommand::BackendReadyToReply(std::shared_ptr<BackendConn> backend,
                                             bool success) {
...
void RedisGetsCommand::OnBackendReplyReceived(std::shared_ptr<BackendConn> backend, ErrorCode ec) {
...
void RedisGetsCommand::OnBackendConnectError(std::shared_ptr<BackendConn> backend) {
  LOG_DEBUG << "RedisGetsCommand::OnBackendConnectError endpoint="
  ...
  if (backend == last_backend_) {
    ...
    LOG_WARN << "RedisGetsCommand::OnBackendConnectError last, endpoint="
            << backend->remote_endpoint() << " backend=" << backend;
  } else {
    LOG_WARN << "RedisGetsCommand::OnBackendConnectError not last, endpoint="
            << backend->remote_endpoint() << " backend=" << backend;
  }
...
/*
void RedisGetsCommand::OnWriteQueryFinished(std::shared_ptr<BackendConn> backend, ErrorCode ec) {
  ...
  LOG_DEBUG << "RedisGetsCommand::OnWriteQueryFinished 转发了当前命令, 等待backend的响应.";
  backend->ReadReply();
}
*/

void RedisGetsCommand::StartWriteReply() {
  NextBackendStartReply();
}

void RedisGetsCommand::NextBackendStartReply() {
  LOG_DEBUG << "RedisGetsCommand::OnWriteReplyEnabled cmd=" << this
  ...
  if (waiting_reply_queue_.size() > 0) {
    ...
    LOG_DEBUG << "RedisGetsCommand::OnWriteReplyEnabled activate ready backend,"
    ...
    LOG_DEBUG << "RedisGetsCommand::OnWriteReplyEnabled no ready backend to activate,"
              << " last_replying_backend_=" << replying_backend_;
    replying_backend_ = nullptr;
  }
}

bool RedisGetsCommand::HasUnfinishedBanckends() const {
  LOG_DEBUG << "RedisGetsCommand::HasUnfinishedBanckends"
...
void RedisGetsCommand::RotateReplyingBackend(bool success) {
  if (success) {
    ++completed_backends_;
  }
  if (HasUnfinishedBanckends()) {
    LOG_DEBUG << "RedisGetsCommand::Rotate to next backend";
    NextBackendStartReply();
  } else {
    LOG_DEBUG << "RedisGetsCommand::Rotate to next COMMAND";
    client_conn_->RotateReplyingCommand();
  }
}

bool RedisGetsCommand::ParseReply(std::shared_ptr<BackendConn> backend) {
