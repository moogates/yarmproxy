...
#for body_size in 50; do
  ...
  # for server in "yarmproxy 11311";do
    ...
    time for id in `seq 1 500`; do cat set_req_${body_size}.tmp | $YARMNC 127.0.0.1 $port > /dev/null 2>&1; done
    #time for id in `seq 1 1`; do cat set_req_${body_size}.tmp | $YARMNC 127.0.0.1 $port >> redis_set.$body_size.$name.tmp 2>&1; done
  done
  round=$((round+1))
  echo
  echo
